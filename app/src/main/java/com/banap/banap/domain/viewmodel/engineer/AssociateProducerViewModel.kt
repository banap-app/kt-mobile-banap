...
import com.banap.banap.domain.use_case.engineer.AssociateProducerUseCase
...
@HiltViewModel
class AssociateProducerViewModel @Inject constructor(
    private val associateProducerUseCase: AssociateProducerUseCase
) : ViewModel() {
    private val _state = mutableStateOf((EngineerState()))
    val state: State<EngineerState> = _state

    fun associateProducer(email: String) {
        associateProducerUseCase(email).onEach { result ->
            when (result) {
                is Resource.Success -> {
