...
import com.banap.banap.domain.use_case.engineer.CreateEngineerUseCase
...
@HiltViewModel
class CreateEngineerViewModel @Inject constructor(
    private val createEngineerUseCase: CreateEngineerUseCase
) : ViewModel() {
    private val _state = mutableStateOf((EngineerState()))
    val state: State<EngineerState> = _state

    fun createEngineer(name: String, email: String, password: String, crea: String) {
        createEngineerUseCase(name, email, password, crea).onEach { result ->
            when (result) {
                is Resource.Success -> {
