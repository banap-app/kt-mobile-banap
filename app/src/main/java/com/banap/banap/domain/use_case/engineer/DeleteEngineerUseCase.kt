...
class DeleteEngineerUseCase @Inject constructor(
    private val repository: EngineerRepositoryImpl
) {
    operator fun invoke(): Flow<Resource<EngineerResponse>> = flow {
        try {
            emit(Resource.Loading())
            val response = repository.deleteEngineer()
            ...
            if (response.isSuccessful) {
                emit(
                    Resource.Success(
                        EngineerResponse(
                            statusCode = 204,
                            message = "Engenheiro deletado com sucesso"
                        )
                    )
                )
            } else {
